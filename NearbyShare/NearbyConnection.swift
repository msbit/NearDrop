...
class NearbyConnection {
  ...
  private static let dispatchQueue = DispatchQueue(
    label: "me.grishka.NearDrop.queue",
    qos: .utility
  ...
  private func receiveFrameAsync() {
    connection.receive(minimumIncompleteLength: 4, maximumLength: 4) {
      ...
      let frameLength: UInt32 =
        UInt32(content[0]) << 24 | UInt32(content[1]) << 16 | UInt32(content[2]) << 8
        | UInt32(content[3])
  ...
  internal func encryptAndSendOfflineFrame(
  ...
  internal func sendTransferSetupFrame(_ frame: Sharing_Nearby_Frame) throws {
    try sendBytesPayload(
      data: try frame.serializedData(), id: Int64.random(in: Int64.min...Int64.max))
  }
  ...
  internal func decryptAndProcessReceivedSecureMessage(_ smsg: Securemessage_SecureMessage) throws {
    ...
    guard offlineFrame.hasV1, offlineFrame.v1.hasType else {
      throw NearbyError.requiredFieldMissing
    }
    ...
    if case .payloadTransfer = offlineFrame.v1.type {
      ...
      if case .bytes = header.type {
        ...
        guard chunk.offset == buffer.count else {
          payloadBuffers.removeValue(forKey: payloadID)
          throw NearbyError.protocolError(
            "Unexpected chunk offset \(chunk.offset), expected \(buffer.count)")
        }
        if chunk.hasBody {
  ...
  internal func finalizeKeyExchange(peerKey: Securemessage_GenericPublicKey) throws {
    ...
    let key = try ECPublicKey(
      domain: domain, w: Point(BInt(magnitude: [UInt8](clientX)), BInt(magnitude: [UInt8](clientY)))
    )
    ...
    let authString = HKDF<SHA256>.deriveKey(
      ...
      salt: "UKEY2 v1 auth".data(using: .utf8)!, info: ukeyInfo, outputByteCount: 32)
    let nextSecret = HKDF<SHA256>.deriveKey(
      inputKeyMaterial: SymmetricKey(data: derivedSecretKey),
      salt: "UKEY2 v1 next".data(using: .utf8)!, info: ukeyInfo, outputByteCount: 32)
    ...
    let d2dClientKey = HKDF<SHA256>.deriveKey(
      ...
      salt: salt,
      info: "client".data(using: .utf8)!,
      outputByteCount: 32
    )
    let d2dServerKey = HKDF<SHA256>.deriveKey(
      inputKeyMaterial: nextSecret,
      salt: salt,
      info: "server".data(using: .utf8)!,
      outputByteCount: 32
    )
    ...
    let clientKey = HKDF<SHA256>.deriveKey(
      ...
      salt: smsgSalt,
      info: "ENC:2".data(using: .utf8)!,
      outputByteCount: 32
    ).withUnsafeBytes({ return [UInt8]($0) })
    let clientHmacKey = HKDF<SHA256>.deriveKey(
      inputKeyMaterial: d2dClientKey,
      salt: smsgSalt,
      info: "SIG:1".data(using: .utf8)!,
      outputByteCount: 32
    )
    ...
    let serverHmacKey = HKDF<SHA256>.deriveKey(
      inputKeyMaterial: d2dServerKey,
      salt: smsgSalt,
      info: "SIG:1".data(using: .utf8)!,
      outputByteCount: 32
