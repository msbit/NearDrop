...
class OutboundNearbyConnection: NearbyConnection {
  ...
  override func processReceivedFrame(frameData: Data) {
    do {
      ...
      case .sentUkeyClientInit:
        try processUkey2ServerInit(
          frame: try Securegcm_Ukey2Message(serializedData: frameData), raw: frameData)
      case .sentUkeyClientFinish:
        try processConnectionResponse(
          frame: try Location_Nearby_Connections_OfflineFrame(serializedData: frameData))
      default:
  ...
  override func processTransferSetupFrame(_ frame: Sharing_Nearby_Frame) throws {
    if frame.hasV1 && frame.v1.hasType, case .cancel = frame.v1.type {
      ...
      try sendDisconnectionAndDisconnect()
      delegate?.outboundConnection(
        connection: self,
        failedWithError: NearbyError.canceled(reason: .userCanceled)
      )
      return
    }
    print(frame)
  ...
  private func sendConnectionRequest() throws {
    ...
    frame.v1.connectionRequest.endpointID = String(
      bytes: NearbyConnectionManager.shared.endpointID, encoding: .ascii)!
    frame.v1.connectionRequest.endpointName = Foundation.Host.current().localizedName!
    let endpointInfo = EndpointInfo(name: Foundation.Host.current().localizedName!, deviceType: .computer)
      ...
      for url in urlsToSend {
        ...
        queue.append(
          OutgoingFileTransfer(
            url: url, payloadID: meta.payloadID, handle: try FileHandle(forReadingFrom: url),
  ...
  private func processConsent(frame: Sharing_Nearby_Frame) throws {
    guard frame.version == .v1, frame.v1.type == .response else {
      throw NearbyError.requiredFieldMissing
    }
    ...
    case .reject, .unknown:
      delegate?.outboundConnection(
        connection: self, failedWithError: NearbyError.canceled(reason: .userRejected))
      try sendDisconnectionAndDisconnect()
    case .notEnoughSpace:
      delegate?.outboundConnection(
        connection: self, failedWithError: NearbyError.canceled(reason: .notEnoughSpace))
      try sendDisconnectionAndDisconnect()
    case .timedOut:
      delegate?.outboundConnection(
        connection: self, failedWithError: NearbyError.canceled(reason: .timedOut))
      try sendDisconnectionAndDisconnect()
    case .unsupportedAttachmentType:
      delegate?.outboundConnection(
        connection: self, failedWithError: NearbyError.canceled(reason: .unsupportedType))
      try sendDisconnectionAndDisconnect()
    }
  }
  ...
  private func sendNextFileChunk() throws {
    ...
    try encryptAndSendOfflineFrame(
      wrapper,
      completion: {
        do {
    ...
    delegate?.outboundConnection(
      connection: self,
      transferProgress: Double(totalBytesSent) / Double(totalBytesToSend)
    )
