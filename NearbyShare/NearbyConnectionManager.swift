...
struct EndpointInfo {
  ...
  init?(data: Data) {
    ...
    guard let deviceName = String(data: data[18..<(18 + deviceNameLength)], encoding: .utf8) else {
      return nil
    }
  ...
  OutboundNearbyConnectionDelegate
{
  ...
  private static func generateEndpointID() -> [UInt8] {
    var id: [UInt8] = []
    let alphabet = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".compactMap {
      UInt8($0.asciiValue!)
    }
    for _ in 0...3 {
  ...
  private func maybeAddFoundDevice(service: NWBrowser.Result) {
    ...
    let deviceType = RemoteDeviceInfo.DeviceType.fromRawValue(
      value: (Int(endpointInfo[0]) >> 1) & 7)
    let deviceNameLength = Int(endpointInfo[17])
    guard endpointInfo.count >= deviceNameLength + 17 else { return }
    guard
      let deviceName = String(
        data: endpointInfo.subdata(in: 18..<(18 + deviceNameLength)),
        encoding: .utf8
      )
    else { return }
